...
use crate::ui::{choose_from_list, clear_combat_health, narrate, pause, set_combat_health, set_player_health};
use super::actions;
...
pub(crate) fn investigate_threat(state: &mut GameState) -> std::io::Result<()> {
    ...
    loop {
        ...
        if encounter.enemy_hp <= 0 {
            ...
            state.threat.clear();
            if let Some(loc) = state.world.location_by_id_mut(location.id) { loc.dangerous = false; }
            ...
            let trophy = Item { id: encounter.enemy_id, name: trophy_name.clone(), description: format!("A proof that the {} was confronted and survived.", location.name) };
            state.character.inventory.push(trophy.clone());
            notify_item_gain(state, &trophy);
            actions::update_faction_memory_for_location(state, location.id, format!("{} was cleared of danger.", location.name));
        ...
        match choose_from_list("Combat action", &choices, None)? {
            Some(0) => {
                ...
                let character_name = state.character.display_name();
                state.world.record_history(state.character.turn, format!("{} struck {} for {} damage.", character_name, encounter.enemy_name, damage));
                if encounter.enemy_hp > 0 {
            ...
            Some(1) => {
                ...
                let character_name = state.character.display_name();
                state.world.record_history(state.character.turn, format!("{} guarded against {}.", character_name, encounter.enemy_name));
                println!("You guard. Incoming damage is reduced to {}.", retaliation);
                if retaliation > 0 { take_combat_damage(state, retaliation, &encounter.enemy_name, &location.name); }
            }
            Some(2) => {
                actions::advance_time(state, 1);
                state.character.turn += 1;
                let character_name = state.character.display_name();
                state.world.record_history(state.character.turn, format!("{} fled from {} at {}.", character_name, encounter.enemy_name, location.name));
        ...
        if state.character.hp <= 0 {
            ...
            actions::mark_character_dead(state, format!("{} overcame them", encounter.enemy_name), &location_name);
...
fn encounter_profile(state: &GameState, location_name: &str) -> (String, i32, i32, String) {
    if let Some(profile) = state.campaign_content.as_ref().and_then(|content| content.encounter_for(location_name)) {
        (profile.enemy_name.clone(), profile.enemy_hp, profile.enemy_power, profile.trophy_item_name.clone())
    } else {
...
fn notify_item_gain(state: &GameState, item: &Item) {
    ...
    if let Some(art) = state.campaign_content.as_ref().and_then(|content| content.item_art_for(&item.name)) {
        println!("");
        println!("{}", art);
    }
}
...
fn take_combat_damage(state: &mut GameState, damage: i32, enemy_name: &str, location_name: &str) {
    if damage <= 0 { narrate("The blow glances off harmlessly."); return; }
    state.character.hp -= damage;
    set_player_health(state.character.hp, state.character.max_hp);
    let character_name = state.character.display_name();
    state.world.record_history(state.character.turn, format!("{} took {} damage from {} at {}.", character_name, damage, enemy_name, location_name));
